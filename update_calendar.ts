    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        const dayEvents = events.filter(e => {
          ...
          try {
            // try to parse ISO or format "dd MMM yyyy" -> we should probably stick to ISO format or store date properly.
            // Let's assume e.date is a string in format "YYYY-MM-DD"
          ...
          >
            <div className="flex justify-between items-start">
              <span className={\`text-sm font-semibold \${
                isSameDay(day, new Date()) ? 'w-6 h-6 bg-[#531FFF] text-white rounded-full flex items-center justify-center' :
...
const calendarSection = `
            <div className="flex items-center justify-between mb-6">
              ...
              <h2 className="text-xl font-bold text-gray-900">{format(currentDate, 'MMMM yyyy', { locale: idLocale })}</h2>
