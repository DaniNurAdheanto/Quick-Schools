...
export default function AttendancePage() {
  ...
  // Fetch Firestore attendance or merge fallback & user identity
  useEffect(() => {
      ...
      if (user) {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            setStudentInfo({
              ...
              className: data.className || data.kelas || "10 IPA 1"
            });
          } else {
            setStudentInfo({
              id: user.uid,
              name: user.displayName || user.email?.split('@')[0] || "Siswa",
              email: user.email,
              nisn: "NISN-2023001",
              className: "10 IPA 1"
            });
          ...
          console.error("Error fetching user role for attendance page:", err);
        }
        ...
        const unsubscribe = onSnapshot(qAttendance, (snapshot) => {
          if (!snapshot.empty) {
            ...
            // Sort newest first
            firestoreDocs.sort((a: any, b: any) => {
            ...
            setAttendanceData(firestoreDocs);
            setIsRealData(true);
          } else {
  ...
  // Restrict dataset for student role so student ONLY sees own attendance records
  const studentAttendanceData = useMemo(() => {
    if (!isStudentRole) return attendanceData;

    const currentUid = (currentUser?.uid || "").toLowerCase().trim();
    const currentEmail = (currentUser?.email || "").toLowerCase().trim();
    const currentName = (studentInfo?.name || currentUser?.displayName || "").toLowerCase().trim();
    const currentNisn = (studentInfo?.nisn || "").toLowerCase().trim();

    const matched = attendanceData.filter(item => {
      const itemId = (item.studentId || item.uid || "").toString().toLowerCase().trim();
      const itemName = (item.studentName || "").toLowerCase().trim();
      const itemEmail = (item.studentEmail || item.email || "").toLowerCase().trim();

      return (
        (currentUid && itemId === currentUid) ||
        (currentNisn && itemId === currentNisn) ||
        (currentEmail && itemEmail === currentEmail) ||
        (currentName && itemName === currentName) ||
        (currentName && currentName.length > 2 && itemName.includes(currentName))
      );
    });

    if (matched.length > 0) return matched;

    // If real Firestore data is present but student hasn't scanned attendance yet
    if (isRealData) return [];

    // Fallback sample record for student view
    const sName = studentInfo?.name || currentUser?.displayName || currentUser?.email?.split('@')[0] || "Ahmad Rizqi Pratama";
    const sId = studentInfo?.nisn || "NISN-2023001";
    const sClass = studentInfo?.className || "10 IPA 1";
    const sPhoto = currentUser?.photoURL || "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=400&auto=format&fit=crop&q=80";
    ...
    return [
      {
        ...
        faceVerified: true,
        faceMatchScore: 98.5,
        capturedImage: sPhoto,
        location: { lat: -6.200000, lng: 106.816666, distance: 12, inRadius: true },
        status: "Hadir",
        type: "in"
      }
    ];
  }, [attendanceData, isStudentRole, studentInfo, currentUser, isRealData]);
  ...
  }, [studentAttendanceData, searchQuery, statusFilter, classFilter]);
  ...
  const handleExportCSV = () => {
    ...
    if (showInfo) showInfo("Berhasil mengekspor data absensi ke format CSV", "Ekspor Laporan");
  };
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (showInfo) showInfo("Data presensi siswa berhasil disinkronkan dengan AI Server", "Sinkronisasi Data");
    }, 600);
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full space-y-8 animate-in fade-in duration-300">
      ...
      <QuickAttendanceModal
        ...
        studentClass={studentInfo?.className || "10 IPA 1"}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <ScanFace className="w-4.5 h-4.5 text-white animate-pulse" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:border-gray-200 transition-all flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              {isStudentRole ? "Total Presensi Saya" : "Total Absensi"}
            </p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:border-gray-200 transition-all flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              {isStudentRole ? "Hadir Tepat" : "Hadir Tepat"}
            </p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:border-gray-200 transition-all flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              {isStudentRole ? "Presensi Ditolak" : "Absensi Ditolak"}
            </p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:border-gray-200 transition-all flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              {isStudentRole ? "Akurasi AI Saya" : "Akurasi AI"}
            </p>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col">
        ...
        <div className="p-5 border-b border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center bg-gray-50/50">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            {/* Class Filter Dropdown or Badge */}
            {!isStudentRole ? (
              ...
              <div className="px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 shadow-sm flex items-center gap-1.5">
                <Building2 className="w-3.5 h-3.5 text-[#531FFF]" />
                <span>Kelas: {studentInfo?.className || "10 IPA 1"}</span>
              </div>
            )}
